...
@pytest.mark.asyncio
async def test_structured_output_forwards_system_prompt_and_params(
    openai_client, model_id, test_output_model_cls, alist
):
    """Structured output should forward system prompt and model params to responses.parse."""
    model = OpenAIResponsesModel(
        model_id=model_id,
        params={"max_output_tokens": 123, "temperature": 0.2},
    )
    messages = [{"role": "user", "content": [{"text": "Generate a person"}]}]
    mock_response = unittest.mock.Mock(output_parsed=test_output_model_cls(name="Jane", age=28))
    openai_client.responses.parse = unittest.mock.AsyncMock(return_value=mock_response)

    await alist(model.structured_output(test_output_model_cls, messages, system_prompt="SYS"))

    openai_client.responses.parse.assert_called_once_with(
        model=model_id,
        input=[{"role": "user", "content": [{"type": "input_text", "text": "Generate a person"}]}],
        instructions="SYS",
        max_output_tokens=123,
        temperature=0.2,
        text_format=test_output_model_cls,
    )
