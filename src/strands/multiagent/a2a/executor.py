...
try:
    ...
    _A2A_IMPORT_ERROR: ModuleNotFoundError | None = None
except ModuleNotFoundError as exc:
    AgentExecutor = object
    RequestContext = EventQueue = TaskUpdater = Any
    DataPart = FilePart = InternalError = Part = TaskState = TextPart = UnsupportedOperationError = Any
    ServerError = RuntimeError
    _A2A_IMPORT_ERROR = exc

    def new_agent_text_message(*args: Any, **kwargs: Any) -> Any:
        """Raise the optional dependency error for the compatibility shim."""
        _raise_missing_a2a_dependency()

    def new_task(*args: Any, **kwargs: Any) -> Any:
        """Raise the optional dependency error for the compatibility shim."""
        _raise_missing_a2a_dependency()
...
_A2A_IMPORT_ERROR_MESSAGE = (
    "StrandsA2AExecutor requires the optional a2a-sdk dependency. "
    "Install it with `pip install strands-agents[a2a]` or `pip install a2a-sdk`."
)


def _raise_missing_a2a_dependency() -> NoReturn:
    raise ImportError(_A2A_IMPORT_ERROR_MESSAGE) from _A2A_IMPORT_ERROR
